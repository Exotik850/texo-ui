...
pub fn TreeViewItem(
    ...
    rsx!(
        details { open: open(), class: class, aria_disabled: disabled, prevent_default: "onclick",
            summary {
                ...
                div { class: "fill-current w-3 text-center transition duration-[200ms] {c_caret}",
                    if has_child {
                        svg {
...
pub fn FileTreeView<P: AsRef<Path> + Clone + PartialEq + 'static>(path: P) -> Element {
    rsx! { RecursiveTreeView { components: build_tree(path) } }
}

fn expand(item: TreeOutput, child_class: Option<String>) -> Element {
    match item {
        TreeOutput::Item(lead, child) => rsx!(
            TreeViewItem { lead, class: child_class.as_deref().unwrap_or_default(), {child.into_iter().map(|f| expand(f, child_class.clone()))} }
        ),
        TreeOutput::ItemNoChildren(lead) => rsx!( TreeViewItem { lead, class: child_class.unwrap_or_default() } ),
...
pub fn RecursiveTreeView(components: Vec<TreeOutput>, child_class: Option<String>) -> Element {
    rsx! (
        TreeView { {components.into_iter().map(|f| expand(f, child_class.clone()))} }
    )
}
