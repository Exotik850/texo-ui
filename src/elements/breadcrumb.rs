...
pub fn Breadcrumb(
    ...
    #[props(default = "flex".into(), into)] nav_class: String,
    ...
    ol_class: String,
    #[props(default = "Breadcrumb".into(), into)] aria_label: String,
    #[props(extends=nav)] rest_attributes: Vec<Attribute>,
    children: Element,
) -> Element {
    let nav_class = if solid { nav_class } else { solid_class };
    let nav_class = merge_classes(&[&nav_class, &class]);
...
pub fn BreadcrumbItem(
    ...
    let li_class = merge_classes(&["inline-flex items-center", &class]);
